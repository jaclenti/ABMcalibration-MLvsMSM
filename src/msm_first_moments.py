...
class MethodOfMomentsLoss_4Moments(BaseLoss):
    ...
    def __init__(
        ...
        Args:
            ...
            standardise_moments: if True all moments are divided by the absolute value of the real moments,
                default is False.
        """
        MethodOfMomentsLoss_4Moments._validate_covariance_and_calculator(
    ...
    def compute_loss_1d(
        ...
        ensemble_sim_mom_1d = np.array(
            [self._moment_calculator(s)[:4] for s in sim_data_ensemble]
        )

        # compute moments of the real time series
        real_mom_1d = self._moment_calculator(real_data)[:4]
...
class MethodOfMomentsLoss_9Moments(BaseLoss):
    """Class for the 'method of moments' loss."""
    ...
    def __init__(
        ...
        Args:
            ...
            standardise_moments: if True all moments are divided by the absolute value of the real moments,
                default is False.
        """
        MethodOfMomentsLoss_9Moments._validate_covariance_and_calculator(
            moment_calculator, covariance_mat
        )
    ...
    def compute_loss_1d(
        ...
        # compute the moments for the simulated ensemble
        ensemble_sim_mom_1d = np.array(
            [self._moment_calculator(s)[:9] for s in sim_data_ensemble]
        )

        # compute moments of the real time series
        real_mom_1d = self._moment_calculator(real_data)[:9]
